...
BOOL ScrCmd_MartBuy(ScriptContext *ctx) {
    ...
    items[nitems] = 0xFFFF;
    Mart_InitUI(ctx->taskman, ctx->fieldSystem, items, 0, 0, 0, 0);
    return TRUE;
}
...
BOOL ScrCmd_MartSell(ScriptContext *ctx) {
    ...
    dummy[0] = 0xFFFF;
    Mart_InitUI(ctx->taskman, ctx->fieldSystem, dummy, 0, 1, 0, 0);
    return TRUE;
}
...
BOOL ScrCmd_SpecialMartBuy(ScriptContext *ctx) {
    ...
    Mart_InitUI(ctx->taskman, ctx->fieldSystem, _0210FA3C[which], 0, 0, 0, NULL);
    return TRUE;
}
...
BOOL ScrCmd_DecorationMart(ScriptContext *ctx) {
    ...
    which = ScriptGetVar(ctx);
    Mart_InitUI(ctx->taskman, ctx->fieldSystem, _0210F9CC[which], 1, 0, which <= 1 ? 1 : 0, NULL);
    return TRUE;
}
...
BOOL ScrCmd_SealMart(ScriptContext *ctx) {
    ...
    which = ScriptGetVar(ctx);
    Mart_InitUI(ctx->taskman, ctx->fieldSystem, _0210F9E8[which], 2, 0, 0, NULL);
    return TRUE;
}
...
BOOL ScrCmd_771(ScriptContext *ctx) {
    ...
    if (Pokedex_GetNatDexFlag(Save_Pokedex_Get(ctx->fieldSystem->saveData))) {
        Mart_InitUI(ctx->taskman, ctx->fieldSystem, _0210F9CC[date.week], 3, 0, 0, _0210FA04[date.week + 7]);
    } else {
        Mart_InitUI(ctx->taskman, ctx->fieldSystem, _0210F9CC[date.week], 3, 0, 0, _0210FA04[date.week]);
    }
    return TRUE;
}
...
BOOL ScrCmd_772(ScriptContext *ctx) {
    ...
    Mart_InitUI(ctx->taskman, ctx->fieldSystem, _0210F9CC[0], 4, 0, 0, _0210F9D4[i / 6]);
    return TRUE;
}
